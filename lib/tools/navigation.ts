...
export const getNavigationLinks = tool({
    ...
    execute: async ({ resource, action, id }: z.infer<typeof getNavigationLinksParameters>) => {
        ...
        if (action === 'create') {
            ...
            if (normalizedResource === 'meeting') return { url: '/new-meeting' };
            if (normalizedResource === 'attendee') return { url: '/attendees' };
        }
        ...
        if (action === 'update') {
            ...
            if (normalizedResource === 'meeting') return { url: `/dashboard?meetingId=${id}` };
            if (normalizedResource === 'attendee') return { url: `/attendees?attendeeId=${id}` };
        }

        if (action === 'read') {
            if (!id) return { url: normalizedResource === 'meeting' ? '/dashboard' : '/attendees' };
